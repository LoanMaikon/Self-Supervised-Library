...
from src.full_gather_layer import FullGatherLayer
...
class VICReg():
        ...
        for epoch in range(1, self.optimization_num_epochs + 1):
            ...
            for iteration, (images, _) in enumerate(self.train_dataloader):
                ...
                with torch.amp.autocast(device_type="cuda", dtype=torch.float16):
                    ...
                    x2 = self.projection_head(self.encoder(x2))

                    # Invariance loss
                    invariance_loss = torch.nn.functional.mse_loss(x1, x2)
                    ...
                    std_loss = torch.mean(torch.nn.functional.relu(1 - std_x1)) + torch.mean(torch.nn.functional.relu(1 - std_x2))
                    std_loss /= 2

                    # Covariance loss
                    global_batch_size = x1.size(0)
                    cov_x1 = (x1.T @ x1) / (global_batch_size - 1)
                    cov_x2 = (x2.T @ x2) / (global_batch_size - 1)
    ...
    def _load_transform(self):
        ...
        self.transform = v2.Compose([
            ...
            v2.RandomSolarize(p=0.1, threshold=128) if self.data_solarization else v2.Identity(),
            v2.ToImage(),
    ...
    def _load_models(self):
        match self.meta_model_name:
            ...
            case "resnet34":
                self.encoder = resnet34(use_checkpoint=self.meta_checkpoint)

            case "resnet101":
                self.encoder = resnet101(use_checkpoint=self.meta_checkpoint)

            case "resnet50x2":
                self.encoder = resnet50x2(use_checkpoint=self.meta_checkpoint)

            case "resnet50x4":
                self.encoder = resnet50x4(use_checkpoint=self.meta_checkpoint)

            case "resnet50x5":
                self.encoder = resnet50x5(use_checkpoint=self.meta_checkpoint)

            case "resnet200x2":
                self.encoder = resnet200x2(use_checkpoint=self.meta_checkpoint)
        
        self.projection_head = projection_head(projection_head_dims=self.meta_projector, in_dim=self.encoder.get_output_dim())
    ...
    def _load_config(self):
        ...
        self.data_crop_scale = [float(x) for x in self.config["data"]["crop_scale"]]
        self.data_crop_ratio = [float(x) for x in self.config["data"]["crop_ratio"]]
