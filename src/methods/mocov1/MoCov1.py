...
class MoCov1():
        ...
        for epoch in range(1, self.optimization_num_epochs + 1):
            ...
            for iteration, (images, _) in enumerate(self.train_dataloader):
                ...
                with torch.amp.autocast(device_type="cuda", dtype=torch.float16):
                    output, target = self.model(im_q=x1, im_k=x2)
    ...
    def _load_models(self):
        match self.meta_model_name:
            case "resnet50":
                self.model = resnet50(use_checkpoint=self.meta_checkpoint, dim=self.meta_projection_dim, ema=self.optimization_ema, queue_size=self.meta_queue_size, temp=self.optimization_temperature)
    ...
    def _load_config(self):
        ...
        self.data_crop_scale = list(self.config["data"]["crop_scale"])
        self.data_crop_ratio = list(self.config["data"]["crop_ratio"])
        ...
        self.data_normalize_mean = list(self.config["data"]["normalize"]["mean"])
        self.data_normalize_std = list(self.config["data"]["normalize"]["std"])
        ...
        self.meta_queue_size = int(self.config["meta"]["queue_size"])

        self.optimization_ipe_scale = float(self.config["optimization"]["ipe_scale"])
        self.optimization_ema = float(self.config["optimization"]["ema"])
        self.optimization_lr = list(self.config["optimization"]["lr"])
        self.optimization_weight_decay = list(self.config["optimization"]["weight_decay"])
