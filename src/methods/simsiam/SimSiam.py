...
class SimSiam():
        ...
        for epoch in range(1, self.optimization_num_epochs + 1):
            ...
            for iteration, (images, _) in enumerate(self.train_dataloader):
                ...
                with torch.amp.autocast(device_type="cuda", dtype=torch.float16):
                    ...
                    loss = -(self.apply_criterion(p1, z2).mean() + self.apply_criterion(p2, z1).mean()) * 0.5
    ...
    def apply_criterion(self, p, z):
        return self.criterion(p, z)
    ...
    def _load_models(self):
        match self.meta_model_name:
            case "resnet50":
                self.model = resnet50(use_checkpoint=self.meta_checkpoint, dim=self.meta_out_dim, pred_dim=self.meta_pred_dim)
    ...
    def _load_config(self):
        ...
        self.data_crop_ratio = tuple(self.config["data"]["crop_ratio"])
        ...
        self.data_normalize_mean = tuple(self.config["data"]["normalize"]["mean"])
        self.data_normalize_std = tuple(self.config["data"]["normalize"]["std"])
        ...
        self.meta_out_dim = int(self.config["meta"]["out_dim"])
        self.meta_pred_dim = int(self.config["meta"]["pred_dim"])

        self.optimization_ipe_scale = float(self.config["optimization"]["ipe_scale"])
        self.optimization_lr = self.config["optimization"]["lr"]
        self.optimization_weight_decay = self.config["optimization"]["weight_decay"]
