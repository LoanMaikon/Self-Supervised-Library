...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_encoder(self, x, mask_ratio):
        ...
        for blk in self.blocks:
            x = blk(x) if not self.use_checkpoint else torch.utils.checkpoint.checkpoint(blk, x)
        x = self.norm(x)
...
def mae_vit_tiny_patch16(image_size, use_checkpoint):
    return MaskedAutoencoderViT(
        ...
        decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), img_size=image_size, use_checkpoint=use_checkpoint)

def mae_vit_small_patch16(image_size, use_checkpoint):
    return MaskedAutoencoderViT(
        patch_size=16, embed_dim=384, depth=12, num_heads=6,
        decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), img_size=image_size, use_checkpoint=use_checkpoint)

def mae_vit_base_patch16(image_size, use_checkpoint):
    return MaskedAutoencoderViT(
        patch_size=16, embed_dim=768, depth=12, num_heads=12,
        decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), img_size=image_size, use_checkpoint=use_checkpoint)

def mae_vit_large_patch16(image_size, use_checkpoint):
    return MaskedAutoencoderViT(
        patch_size=16, embed_dim=1024, depth=24, num_heads=16,
        decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), img_size=image_size, use_checkpoint=use_checkpoint)

def mae_vit_huge_patch14(image_size, use_checkpoint):
    return MaskedAutoencoderViT(
