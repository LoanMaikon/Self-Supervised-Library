# Code from https://github.com/facebookresearch/dinov2/blob/main/dinov2/models/vision_transformer.py
...
try:
    if XFORMERS_ENABLED:
        ...
        print("xFormers is available (SwiGLU)")
    else:
        print("xFormers is disabled (SwiGLU)")
        raise ImportError
except ImportError:
    SwiGLU = SwiGLUFFN
    XFORMERS_AVAILABLE = False

    print("xFormers is not available (SwiGLU)")
...
class DINOHead(nn.Module):
    ...
    def cancel_gradients_last_layer(self, epoch, freeze_last_layer_epochs):
        if epoch <= freeze_last_layer_epochs:
            self.last_layer.weight_g.grad = None
